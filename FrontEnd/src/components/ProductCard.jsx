  ...
  return (
    <div className="card flex flex-col justify-center bg-white items-center m-2 p-2 rounded relative group" style={{ width: "18rem" }}>
      ...
      <div className="absolute w-full hidden group-hover:block bottom-0">
        ...
        <button className="bg-green-400 h-10 w-full p-2 text-white hover:bg-green-600 active:scale-95 transition-all ease-in-out" onClick={() => setEditProductCard(true)}>
          Edit
        </button>
        <button className="bg-red-400 h-10 w-full p-2 text-white hover:bg-red-600 active:scale-95 transition-all ease-in-out"  onClick={() => setDeleteProduct(true)}>
          Delete
        </button>
        </div>
      </div>
    ...
    {editProductCard && (<AdminEditProduct item = {item} onClose={() => setEditProductCard(false)}  />) }
    {deleteProduct && <AdminDeleteProduct itemDelete={item} onDelete={() => setDeleteProduct(false)} />}
    </div>
  );
};
